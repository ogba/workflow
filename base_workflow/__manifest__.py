{
    'name': "Workflow",
    'version': '17.0',
    'sequence': 1,
    'category': 'Asas/Administration',
    'summary': 'Manage dynamic workflow',
    'description': """ manage and automate sequential tasks or processes, improving efficiency and collaboration within a system or organization.
            It provides a structured framework for defining, executing, and monitoring workflows, 
            ensuring smooth and streamlined operations.""",

    'author': 'Aztek computers',
    'company': 'Aztek computers',
    'maintainer': 'Aztek computers',
    'website': "https://www.aztekcomputers.com",
    'depends': ['base', 'mail', 'hr','account','base_model_filters', 'base_validation_error_messages'],
    'data': [
        'security/security.xml',
        'security/ir.model.access.csv',
        'data/ir_sequence.xml',
        'data/mail_template.xml',
        'data/ir_cron.xml',
        'wizards/approval_wizard_views.xml',
        'wizards/forward_wizard_views.xml',
        'views/workflow_views.xml',
        'views/res_config_settings_views.xml',
        'views/workflow_dashboard_views.xml',
        'views/workflow_category_view.xml',
        'views/workflow_dashboard_view.xml',
        'views/workflow_delegate_view.xml',
        'views/workflow_sla_view.xml',
        'views/workflow_approval.xml',
        'views/workflow_request_view.xml',
    ],

    'assets': {
            'web.assets_backend': [
                'base_workflow/static/src/css/dashboard.css',
                'base_workflow/static/src/css/style.scss',
                'base_workflow/static/src/css/material-gauge.css',
                'base_workflow/static/src/js/workflow_dashboard.js',
                'base_workflow/static/src/js/lib/highcharts.js',
                'base_workflow/static/src/js/lib/Chart.bundle.js',
                'base_workflow/static/src/js/lib/funnel.js',
                'base_workflow/static/src/js/lib/d3.min.js',
                'base_workflow/static/src/js/lib/material-gauge.js',
                'base_workflow/static/src/js/lib/columnHeatmap.min.js',
                'base_workflow/static/src/js/lib/columnHeatmap.js',
                'base_workflow/static/src/xml/dashboard_templates.xml',
            ],
        },

    'images': ['static/description/banner.png'],
    'license': 'OPL-1',
	'currency': 'EUR',
	'price': '389',
    'installable': True,
    'auto_install': False,
    'application': True,
}
